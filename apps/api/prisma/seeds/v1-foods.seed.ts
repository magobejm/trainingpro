...
type FoodType = 'ingrediente' | 'plato';
type ServingUnit = '100g' | '100ml' | 'porcion';
...
const FOODS_100ML: Array<FoodSource & { foodCategory: FoodCategory }> = [
...
const FOODS_PORTION: Array<FoodSource & { foodCategory: FoodCategory; notes: string }> = [
...
export const FOODS_V1: FoodSeed[] = [
  ...
  ...FOODS_PORTION.map((item, index) => ({
    ...
    servingUnit: 'porcion' as const,
  })),
];

function resolve100gCategory(position: number): FoodCategory {
  if (position <= 3) return 'aceites_y_grasas';
  if (position <= 20) return 'verduras_hortalizas';
  if (position <= 25) return 'legumbres';
  if (position <= 32) return 'cereales_tuberculos';
  if (position <= 43) return 'frutas';
  if (position <= 50) return 'frutos_secos_semillas';
  if (position <= 58) return 'lacteos';
  if (position <= 65) return 'carnes_huevos';
  if (position <= 76) return 'pescados_mariscos';
  if (position <= 81) return 'cereales_tuberculos';
  if (position <= 86) return 'salsas_y_cremas';
  if (position <= 93) return 'dulces_y_desayuno';
  if (position <= 98) return 'frutas';
  return 'pescados_mariscos';
}

function resolve100gType(position: number): FoodType {
