...
const nutrientSchema = z.number().min(0).max(2000).nullable().optional();
...
export class CreateFoodDto {
  static schema = z
    .object({
      ...
      micronutrients: z.array(z.string().trim().min(1).max(80)).max(20).optional(),
      name: z.string().trim().min(1).max(120),
      notes: z.string().max(2000).nullable().optional(),
      proteinG: macroSchema,
      saltG: nutrientSchema,
      saturatedFatG: nutrientSchema,
      servingUnit: servingUnitSchema,
      sugarG: nutrientSchema,
      unsaturatedFatG: nutrientSchema,
    })
