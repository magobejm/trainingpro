...
import { NOTES_REPOSITORY, type INotesRepository, type UpdateNoteInput } from '../../domain/notes.repository.port';
...
export class UpdateNoteUseCase {
  ...
  async execute(context: AuthContext, noteId: string, input: UpdateNoteInput) {
    if (context.activeRole !== 'coach') {
      throw new ForbiddenException('Only coach can update notes');
    }
    const membership = await this.readCoachMembership(context.subject);
    return this.notesRepository.update(noteId, membership.id, input);
  }
