...
export function normalizeAdvancedTechniqueId(stored?: null | string): AdvancedTechniqueId {
...
export function advancedTechniqueDescription(stored: string, t: (k: string) => string): string {
  const id = normalizeAdvancedTechniqueId(stored);
  const key = `coach.routine.advancedTechnique.${id}.desc`;
  const tr = t(key);
  return tr === key ? '' : tr;
}
